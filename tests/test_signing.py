...
# Canonical 40-char hex fingerprint for all test mocks
TEST_FP = "A1B2C3D4E5F6A7B8C9D0E1F2A3B4C5D6E7F8A9B0"
...
class TestKeyGeneration:
    ...
    def test_generate_agent_key_already_exists(self, mock_keyring, mock_gpg_commands):
        ...
        # Mock existing key
        mock_keyring.get_password.return_value = TEST_FP
    ...
    def test_generate_agent_key_invalid_fingerprint_format(
        self, mock_keyring, mock_gpg_commands
    ):
        """Test failure when GPG returns non-hex fingerprint."""
        mock_keyring.get_password.return_value = None
        mock_gpg_commands.return_value = (
            "[GNUPG:] KEY_CREATED B NOTAHEXFINGERPRINT\n",
            "",
            0,
        )

        with pytest.raises(GPGKeyGenerationError, match="Invalid fingerprint format"):
            generate_agent_key()

    def test_generate_agent_key_wrong_length_fingerprint(
        self, mock_keyring, mock_gpg_commands
    ):
        """Test failure when GPG returns hex but wrong length fingerprint."""
        mock_keyring.get_password.return_value = None
        mock_gpg_commands.return_value = (
            "[GNUPG:] KEY_CREATED B " + "A" * 39 + "\n",
            "",
            0,
        )
    ...
    def test_generate_agent_key_keyring_storage_fails(
        ...
        # Status output with valid 40-char hex fingerprint
        mock_gpg_commands.return_value = (
            f"[GNUPG:] KEY_CREATED B {TEST_FP}\n",
            "",
            0,
...
class TestGitConfiguration:
    ...
    def test_configure_git_signing_secret_key_mismatch(
        ...
        """Test failure when stored fingerprint doesn't match any GPG secret key."""
        mock_keyring.get_password.return_value = TEST_FP
        # GPG returns a different fingerprint in its output
        mock_gpg_commands.return_value = (
        ...
        with pytest.raises(GPGError, match="secret key not found"):
            configure_git_signing(project_root=tmp_path)
