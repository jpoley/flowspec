...
class TestSecurityProtections:
    ...
    def test_non_zip_archive_extraction_with_file_limit(self, tmp_path):
        """FIX #4 & #7: Test tar.gz file extraction with file count limit."""
        import io
        import tarfile
    ...
    def test_tar_gz_file_limit_exceeded(self, tmp_path):
        """FIX #7: Test that tar.gz files exceeding MAX_ARCHIVE_FILES are rejected."""
        import io
        import tarfile
        ...
        # Create a tar.gz with too many files
        large_tar = tmp_path / "large.tar.gz"
        with tarfile.open(large_tar, "w:gz") as tf:
            for i in range(manager.MAX_ARCHIVE_FILES + 10):
                content = f"content {i}".encode()
                tarinfo = tarfile.TarInfo(name=f"file{i}.txt")
                tarinfo.size = len(content)
        ...
        # Should raise RuntimeError for too many files
        with pytest.raises(RuntimeError, match="too many files"):
            manager._safe_extract_archive(large_tar, extract_dir)

    def test_extension_checking_case_insensitive(self, tmp_path):
        """FIX #4: Extension checking should be case-insensitive."""
        import zipfile
    ...
    def test_path_traversal_bypass_attempts(self, tmp_path):
        """FIX #5: Test various path traversal bypass attempts using enumerate()."""
        import zipfile
        ...
        # FIX #5: Use enumerate() instead of .index()
        for i, attempt in enumerate(bypass_attempts):
            malicious_zip = tmp_path / f"evil_{i}.zip"
            with zipfile.ZipFile(malicious_zip, "w") as zf:
    ...
    def test_tar_symlink_with_path_traversal_rejected(self, tmp_path):
        ...
        # Should reject symlink with path traversal
        with pytest.raises(RuntimeError, match="path traversal"):
    ...
    def test_tar_symlink_to_absolute_path_rejected(self, tmp_path):
        ...
        # Should reject absolute symlink
        with pytest.raises(RuntimeError, match="absolute path"):
            manager._safe_extract_archive(malicious_tar, extract_dir)
...
class TestURLValidation:
    ...
    def test_url_template_malformed_after_substitution(self, tmp_path):
        """Malformed URLs after substitution are rejected."""
        manager = ToolManager(cache_dir=tmp_path)

        # This would create https://example.com//tool.zip
        valid, error = manager._validate_url_template(
            "https://example.com//{version}tool.zip",
            "",  # Empty version
        )
        assert (
            ...
            or "version is empty" in error.lower()
        )
...
class TestConcurrentInstallation:
    ...
    def test_concurrent_install_lock(self, tmp_path):
        ...
        def install_with_lock():
            try:
                with manager._acquire_install_lock("test-tool", "1.0.0", timeout=2):
                    # Simulate work
                    time.sleep(0.5)
        ...
        assert any("error" in r or "success" in r for r in results)
