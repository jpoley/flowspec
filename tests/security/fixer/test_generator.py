...
from specify_cli.security.fixer.models import FixStatus
from specify_cli.security.fixer.generator import FixGenerator, FixGeneratorConfig
...
class TestFixGenerator:
    ...
    def test_generate_fix_with_mocked_llm(self, tmp_path):
        ...
        vuln_file.write_text('query = "SELECT * FROM users WHERE id = " + user_id')
    ...
    def test_generate_fix_unknown_cwe(self, tmp_path):
        ...
        vuln_file.write_text("some vulnerable code")
...
class TestMultipleFixes:
    ...
    def test_generate_fixes(self, tmp_path):
        ...
        file1.write_text("vulnerable code 1")

        file2 = tmp_path / "app2.py"
        file2.write_text("vulnerable code 2")

        generator = FixGenerator()
        findings = [
            make_finding(id="F1", file_path=str(file1), cwe_id="CWE-89"),
            make_finding(id="F2", file_path=str(file2), cwe_id="CWE-79"),
        ]
