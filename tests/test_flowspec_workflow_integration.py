"""Integration tests for /flowspec workflow state constraints.

This test module verifies that /flowspec commands correctly enforce workflow
state constraints through the WorkflowStateGuard module. It covers:

- AC #1: Test file exists
- AC #2-7: State transitions for all /flowspec commands
...
def default_workflow_config():
    """Standard workflow configuration matching flowspec_workflow.yml."""
    return {
        "version": "1.1",
        "states": [
            "To Do",
...
def test_integration_test_file_exists():
    ...
    assert test_file.name == "test_flowspec_workflow_integration.py"


# =============================================================================
# AC #2: /flow:specify State Transition (Assessed → Specified)
# =============================================================================


class TestSpecifyTransition:
    """AC #2: Tests for /flow:specify state transition (Assessed → Specified)."""
    ...
    def test_specify_blocked_from_todo(self, workflow_config_file):
        ...
        assert "Assessed" in response.required_states
        assert "/flow:specify" in response.message
...
# =============================================================================
# AC #3: /flow:research State Transition (Specified → Researched)
# =============================================================================


class TestResearchTransition:
    """AC #3: Tests for /flow:research state transition (Specified → Researched)."""
...
# =============================================================================
# AC #4: /flow:plan State Transition (Specified/Researched → Planned)
# =============================================================================


class TestPlanTransition:
    """AC #4: Tests for /flow:plan state transition (Specified/Researched → Planned)."""
...
# =============================================================================
# AC #5: /flow:implement State Transition (Planned → In Implementation)
# =============================================================================


class TestImplementTransition:
    """AC #5: Tests for /flow:implement state transition (Planned → In Implementation)."""
    ...
    def test_implement_blocked_from_specified(self, workflow_config_file):
        ...
        # Should suggest /flow:plan
        assert response.suggested_workflows is not None
        assert "/flow:plan" in response.suggested_workflows
    ...
    def test_implement_error_message_quality(self, workflow_config_file):
        ...
        assert "Cannot run /flow:implement" in response.message
...
# =============================================================================
# AC #6: /flow:validate State Transition (In Implementation → Validated)
# =============================================================================


class TestValidateTransition:
    """AC #6: Tests for /flow:validate state transition (In Implementation → Validated)."""
    ...
    def test_validate_suggests_implement_workflow(self, workflow_config_file):
        """Validate command suggests /flow:implement when task is Planned."""
        guard = WorkflowStateGuard(workflow_config_file)
        response = guard.check_state("validate", "Planned")

        assert response.suggested_workflows is not None
        assert "/flow:implement" in response.suggested_workflows
...
class TestInvalidTransitions:
    ...
    def test_blocked_transition_suggests_valid_workflows(self, workflow_config_file):
        ...
        assert response.suggested_workflows is not None
        assert "/flow:assess" in response.suggested_workflows
...
class TestCustomWorkflowConfig:
    ...
    def test_custom_config_suggestions(self, custom_config_file):
        ...
        assert "/flow:start" in workflows

        workflows = guard.get_valid_workflows_for_state("Active")
        assert "/flow:finish" in workflows
...
class TestEdgeCases:
    ...
    def test_assess_workflow_at_start(self, workflow_config_file):
        ...
        assert valid_workflows == ["/flow:assess"]
