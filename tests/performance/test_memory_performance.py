...
class TestInjectionPerformance:
    ...
    @pytest.fixture
    def injection_setup(self, perf_project, perf_manager):
        """Store with many active tasks for injection testing."""
        # Create backlog/CLAUDE.md for ContextInjector
        backlog_dir = perf_project / "backlog"
        backlog_dir.mkdir(exist_ok=True)
        (backlog_dir / "CLAUDE.md").write_text("""# Backlog Context

Task memory files are stored here.
""")
        ...
        injector = ContextInjector(base_path=perf_project)
        return injector, task_ids

    def test_inject_100_tasks_performance(self, injection_setup):
        """Test injecting 100 active tasks (<100ms target)."""
        injector, task_ids = injection_setup
        idx = 0
        ...
        stats = benchmark_operation(inject_context, iterations=100)
