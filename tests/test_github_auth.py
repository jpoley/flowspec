...
class TestLegacyAssetNamingFallback:
    ...
    def test_legacy_asset_naming_fallback(self, temp_dir):
        ...
        # Simulate an older release with ONLY legacy naming (no new pattern)
        mock_release_response.json.return_value = {
            "tag_name": "v0.3.0",
            "assets": [
                {
                    "name": "spec-kit-template-claude-sh-v0.3.0.zip",
                    "browser_download_url": "https://example.com/legacy-asset.zip",
                    "url": "https://api.example.com/legacy-asset",
                    "size": 1000,
                }
            ],
        }
        ...
        # Verify the legacy asset URL was selected for download
        assert any("legacy-asset" in url for url in download_urls), (
            f"Should have selected legacy asset URL, got: {download_urls}"
        )
