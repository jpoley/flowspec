...
class TestStatusReporting:
    ...
    def test_reports_fail_for_missing_commands(self, script_path, missing_command_config):
...
class TestDocumentation:
    ...
    def test_mcp_config_memory_documents_script(self):
        ...
        mcp_config_path = Path(__file__).parent.parent / "memory" / "mcp-configuration.md"
...
class TestErrorHandling:
    ...
    def test_handles_missing_config_file(self, script_path, tmp_path):
        ...
        result = subprocess.run(
            [str(script_path), "-c", str(nonexistent)],
            capture_output=True,
            text=True
        )
        assert result.returncode == 2
