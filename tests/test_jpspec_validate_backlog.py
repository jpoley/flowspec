...
class TestValidateCommandStructure:
    ...
    def test_validate_includes_backlog_instructions_marker(self, validate_md_path):
        """Verify validate.md includes {{BACKLOG_INSTRUCTIONS}} markers."""
        content = validate_md_path.read_text()

        # Should have 4 occurrences (one per agent: QA, Security, Tech Writer, Release Manager)
        marker_count = content.count("{{BACKLOG_INSTRUCTIONS}}")
        assert marker_count == 4, f"Expected 4 {{{{BACKLOG_INSTRUCTIONS}}}} markers, found {marker_count}"
