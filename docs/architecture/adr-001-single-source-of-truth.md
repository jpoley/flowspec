...
- .claude/commands/flow/implement.md:     19,960 bytes (6.8x larger)
...
**Description**: Continue maintaining enhanced commands in `.claude/commands/flow/` and minimal templates in `templates/commands/flowspec/`.
...
**Description**: Use `.claude/commands/flow/` as the canonical source, copy from there to `templates/` during build.
...
3. **Delete original `.claude/commands/flow/` files** - replace with symlinks created by dev-setup
...
templates/commands/flowspec/implement.md   ◄─── CANONICAL SOURCE (20KB enhanced)
                │
                ├─► (dev-setup symlink) .claude/commands/flow/implement.md
                │
                └─► (init copy) user-project/.claude/commands/flow/implement.md
```
