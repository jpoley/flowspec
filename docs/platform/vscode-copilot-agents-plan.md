...
This plan enables Flowspec to work with VS Code GitHub Copilot in addition to Claude Code CLI. The `.claude/commands/` directory remains the single source of truth, with `.github/agents/` generated via automated transformation including workflow handoffs.
...
|-------|---------------|--------------|
| flowspec-assess | ✓ Assessment Complete → Specify Requirements | flowspec-specify |
| flowspec-specify | ✓ Specification Complete → Conduct Research | flowspec-research |
| flowspec-specify | ✓ Specification Complete → Create Technical Design | flowspec-plan |
| flowspec-research | ✓ Research Complete → Create Technical Design | flowspec-plan |
| flowspec-plan | ✓ Planning Complete → Begin Implementation | flowspec-implement |
| flowspec-implement | ✓ Implementation Complete → Run Validation | flowspec-validate |
| flowspec-validate | ✓ Validation Complete → Deploy to Production | flowspec-operate |
| flowspec-operate | (none - terminal state) | - |
...
1. **Discover**: Find all `.md` files in `.claude/commands/{flowspec,speckit}/`
...
| task-330 | Convert flowspec commands to Copilot format | task-329 |
...
*Generated by /flow:plan on 2025-12-08*
