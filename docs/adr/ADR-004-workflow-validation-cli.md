...
Flowspec's workflow configuration (`flowspec_workflow.yml`) is a critical file that defines:
- Task lifecycle states (To Do, Specified, Planned, etc.)
- Workflow commands (`/flow:specify`, `/flow:implement`, etc.)
...
- Invalid state references cause cryptic errors when executing `/flowspec` commands
...
{
  "valid": true,
  "config_file": "/path/to/flowspec_workflow.yml",
  "schema_validation": {
    "passed": true,
    "error": null
  },
...
3. **Manual Testing**:
   - Real `flowspec_workflow.yml` validation
   - Custom config files
...
- Schema stored at `memory/flowspec_workflow.schema.json`
