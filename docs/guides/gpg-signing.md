...
5. **File Permissions**: Ensure `~/.gnupg/` has restrictive permissions (`chmod 700 ~/.gnupg`). The unprotected private key means anyone with read access to this directory can sign as the agent.
...
gpg --list-secret-keys agent@flowspec.local
```
...
**Important:** Manual GPG deletion does not remove the fingerprint from the system keyring. Use `flowspec gpg rotate` instead, which cleans up both stores. If you must delete manually, run `flowspec gpg setup --force` afterward to restore a consistent state.
...
- [Security Best Practices](#security-best-practices) - Key protection and rotation guidance
- [Key Rotation Workflow](#key-rotation-workflow) - Operational guidance for rotating keys
- [Git Signing Documentation](https://git-scm.com/book/en/v2/Git-Tools-Signing-Your-Work) - Git commit signing reference
