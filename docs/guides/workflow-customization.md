...
The `jpspec_workflow.yml` file is the single source of truth for your development workflow. It defines:
...
**Location**: `{project-root}/jpspec_workflow.yml`
...
1. **Restore from git**:
   ```bash
   git checkout jpspec_workflow.yml
   ```
...
```bash
cp jpspec_workflow.yml jpspec_workflow.yml.backup.$(date +%Y%m%d)
vim jpspec_workflow.yml
```
...
- `jpspec_workflow.yml` is the single source of truth for workflow configuration
