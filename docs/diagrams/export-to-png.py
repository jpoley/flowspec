...
# Configuration
EXCALIDRAW_FILE = Path(__file__).parent / 'jpspec-workflow.excalidraw'
OUTPUT_PNG = Path(__file__).parent / 'jpspec-workflow.png'
...
def draw_rounded_rectangle(draw, x, y, width, height, radius, fill, outline, width_px):
    ...
    # Main rectangles
    draw.rectangle([x + radius, y, x + width - radius, y + height], fill=fill, outline=None)
    draw.rectangle([x, y + radius, x + width, y + height - radius], fill=fill, outline=None)

    # Corners
    draw.ellipse([x, y, x + radius * 2, y + radius * 2], fill=fill, outline=None)
    draw.ellipse([x + width - radius * 2, y, x + width, y + radius * 2], fill=fill, outline=None)
    draw.ellipse([x, y + height - radius * 2, x + radius * 2, y + height], fill=fill, outline=None)
    draw.ellipse([x + width - radius * 2, y + height - radius * 2, x + width, y + height], fill=fill, outline=None)

    # Outline
    if outline:
        # Top and bottom
        draw.line([x + radius, y, x + width - radius, y], fill=outline, width=width_px)
        draw.line([x + radius, y + height, x + width - radius, y + height], fill=outline, width=width_px)

        # Left and right
        draw.line([x, y + radius, x, y + height - radius], fill=outline, width=width_px)
        draw.line([x + width, y + radius, x + width, y + height - radius], fill=outline, width=width_px)

        # Corner arcs
        draw.arc([x, y, x + radius * 2, y + radius * 2], 180, 270, fill=outline, width=width_px)
        draw.arc([x + width - radius * 2, y, x + width, y + radius * 2], 270, 360, fill=outline, width=width_px)
        draw.arc([x, y + height - radius * 2, x + radius * 2, y + height], 90, 180, fill=outline, width=width_px)
        draw.arc([x + width - radius * 2, y + height - radius * 2, x + width, y + height], 0, 90, fill=outline, width=width_px)
...
def render_excalidraw_to_png():
    ...
    sorted_elements = sorted(elements, key=lambda el:
        (0 if el['type'] in ['rectangle', 'diamond'] else 1 if el['type'] == 'arrow' else 2, el.get('id', '')))
    ...
    for el in sorted_elements:
        ...
        if el_type == 'rectangle':
            ...
            if roundness.get('type') == 3:
                # Rounded rectangle
                radius = int(8 * SCALE)
                draw_rounded_rectangle(draw, x, y, w, h, radius, fill_rgb, stroke_rgb, stroke_width)
            else:
                # Regular rectangle
                if fill_rgb:
                    draw.rectangle([x, y, x + w, y + h], fill=fill_rgb)
                if stroke_rgb:
                    draw.rectangle([x, y, x + w, y + h], outline=stroke_rgb, width=stroke_width)
