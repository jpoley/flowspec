...
Implement the foundational task context loading capability for the enhanced /flowspec:validate command. This phase is responsible for loading a backlog task by ID, parsing its acceptance criteria into a structured format, and determining the validation approach (automated vs manual) for each AC based on naming conventions and task metadata.
