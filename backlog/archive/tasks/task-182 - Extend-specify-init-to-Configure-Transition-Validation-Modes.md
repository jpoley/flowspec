...
- [x] AC5: Generate flowspec_workflow.yml with configured validation modes
...
<!-- SECTION:NOTES:BEGIN -->
## Summary
Extend the `specify init` command to ask users about their preferred validation mode for each workflow transition, storing the configuration in flowspec_workflow.yml.
...
1. Assess → Specified (after /flowspec:assess)
   [1] NONE (default)
   [2] KEYWORD
   [3] PULL_REQUEST
   > 1

2. Specified → Researched (after /flowspec:specify, produces PRD)
   [1] NONE (default)
...
3. Researched → Planned (after /flowspec:research)
   [1] NONE (default)
   [2] KEYWORD
   [3] PULL_REQUEST
   > 1

4. Planned → In Implementation (after /flowspec:plan, produces ADRs)
   [1] NONE (default)
   [2] KEYWORD
   [3] PULL_REQUEST
   > 3

5. In Implementation → Validated (after /flowspec:implement)
   [1] NONE (default)
   [2] KEYWORD
   [3] PULL_REQUEST
   > 1

6. Validated → Deployed (after /flowspec:validate)
   [1] NONE (default)
   [2] KEYWORD
   [3] PULL_REQUEST
   > 2
...
Configuration saved to flowspec_workflow.yml
```
...
- [ ] AC5: Generate flowspec_workflow.yml with configured validation modes
