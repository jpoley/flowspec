...
2. Create PM commands (assess, define, discover) - migrate from flow:assess, flow:specify, flow:research
3. Create Arch commands (design, decide, model) - migrate from flow:plan
4. Create Dev commands (build, debug, refactor, cleanup) - migrate from flow:implement
5. Create Sec commands (scan, triage, fix, audit, report) - migrate from flow:security_*
6. Create QA commands (test, verify, review) - migrate from flow:validate
7. Create Ops commands (deploy, monitor, respond, scale) - migrate from flow:operate
