...
1. Create build script:
   ...
   VERSION=${1:-latest}
   IMAGE_NAME="flowspeckit/security-scanner"
...
2. Test image functionality:
   ```bash
   # Test 1: Help command
   docker run --rm flowspeckit/security-scanner:latest --help
   
   # Test 2: Scan current directory
   docker run --rm -v $(pwd):/src flowspeckit/security-scanner:latest
   ...
   docker images flowspeckit/security-scanner:latest --format "{{.Size}}"
   # Target: <200MB
   ```
