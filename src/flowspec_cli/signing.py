...
Agents can generate their own GPG keys and configure git to use them.
"""
...
""".strip()
    ...
    if len(fingerprint) != 40 or not all(c in "0123456789ABCDEF" for c in fingerprint):
        raise GPGKeyGenerationError(
            f"Invalid fingerprint format from GPG: {fingerprint}"
        )
...
def configure_git_signing(project_root: Optional[Path] = None) -> None:
    ...
    fingerprint_found = any(
        line.startswith("fpr:") and line.split(":")[9].upper() == fingerprint
        for line in stdout.splitlines()
    )
...
def get_key_fingerprint() -> Optional[str]:
    ...
    try:
        ...
        if len(fingerprint) != 40:
            logger.warning(
                "Stored fingerprint has invalid length: %d", len(fingerprint)
            )
            return None
        try:
            int(fingerprint, 16)
        except ValueError:
            logger.warning("Stored fingerprint contains non-hex characters")
            return None
        return fingerprint
    except Exception as e:
