...
def generate_jpspec_workflow_yml(project_path: Path, validation_mode: str) -> None:
    """Generate jpspec_workflow.yml with configured validation modes.

    Args:
        project_path: Path to the project directory.
        validation_mode: Default validation mode (none, keyword, pull-request).
    """
    # Map CLI mode to YAML validation value
    mode_map = {
        "none": "NONE",
    ...
    lines = [
        ...
        '# Validation modes: NONE, KEYWORD["<string>"], PULL_REQUEST',
        "",
        'version: "1.0"',
        "",
        "transitions:",
    ...
    for t in transitions:
        ...
        lines.append(f"    validation: {yaml_mode}")
        lines.append("")
...
def init(
    ...
    validation_mode: str = typer.Option(
        "none",
        "--validation-mode",
        help="Default validation mode for workflow transitions: none, keyword, or pull-request",
    ),
    no_validation_prompts: bool = typer.Option(
        False,
        "--no-validation-prompts",
        help="Skip validation mode prompts and use defaults (NONE for all transitions)",
    ),
):
    """
    ...
    console.print()
    console.print(
        f"[green]Generated jpspec_workflow.yml with validation mode: {effective_mode.upper()}[/green]"
    )
