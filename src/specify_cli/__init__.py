...
# When present, specflow init/upgrade will skip to avoid clobbering source files
...
def show_version_info(detailed: bool = False, centered: bool = False) -> None:
    ...
    if detailed:
        ...
        upgrades_available.append(
            _add_version_row(table, "SpecFlow", versions["jp_spec_kit"])
        )
        upgrades_available.append(
        ...
        if any(upgrades_available):
            hint = "[dim]Run 'specflow upgrade-tools' to update CLI tools[/dim]"
            if centered:
                console.print(Align.center(hint))
            else:
                console.print(hint)
    else:
        console.print(f"SpecFlow {versions['jp_spec_kit']['installed']}")
...
def callback(
    ...
    if (
        ...
        console.print()
        console.print(
            Align.center("[dim]Run 'specflow --help' for usage information[/dim]")
        )
        console.print()
...
def init(
    ...
    Examples:
        ...
        specflow init my-project --ai claude,copilot  # Multiple agents
        specflow init my-project --ai copilot --no-git
        specflow init --ignore-agent-tools my-project
        specflow init . --ai claude         # Initialize in current directory
        specflow init . --ai claude,cursor-agent,copilot  # Multiple agents in current dir
        specflow init .                     # Initialize in current directory (interactive AI selection)
        specflow init --here --ai claude    # Alternative syntax for current directory
        specflow init --here --ai codex
        specflow init --here --ai codebuddy,claude
        specflow init --here
        specflow init --here --force  # Skip confirmation when current directory not empty
        specflow init my-project --constitution medium  # Specific constitution tier
        specflow init my-project --ai claude --constitution light  # Startup tier
        specflow init my-project --ai claude --constitution heavy  # Enterprise tier
    """
    ...
    if here:
        ...
        if marker_path.exists():
            error_panel = Panel(
                ...
                "Running 'specflow init' here would overwrite source files.\n\n"
                "[cyan]If you want to use jp-spec-kit commands in this repo:[/cyan]\n"
                "  Run 'specflow dev-setup' to set up for development.\n\n"
                "[cyan]If you want to test 'specflow init' on a new project:[/cyan]\n"
                "  Create a new directory and run 'specflow init' there instead.",
...
def upgrade_repo(
    ...
    It does NOT upgrade the globally installed CLI tools (use 'specflow upgrade-tools' for that).
    ...
    Examples:
        specflow upgrade-repo                               # Upgrade repo to latest templates
        specflow upgrade-repo --dry-run                     # Preview changes
        specflow upgrade-repo --base-version 0.0.20         # Pin base to specific version
        specflow upgrade-repo --extension-version 0.0.21    # Pin extension to specific version
        specflow upgrade-repo --templates-only              # Only update template files

    See also:
        specflow upgrade-tools    # Upgrade globally installed CLI tools
    """
    show_banner()
    ...
    if marker_path.exists():
        error_panel = Panel(
            f"[yellow]This directory contains '{SOURCE_REPO_MARKER}'[/yellow]\n\n"
            "This indicates it is the jp-spec-kit source repository.\n"
            "Running 'specflow upgrade-repo' here would overwrite source files.\n\n"
            "[cyan]To update jp-spec-kit itself:[/cyan]\n"
            "  Use git pull or standard development workflow.\n\n"
            "[cyan]To test 'specflow upgrade-repo' on a project:[/cyan]\n"
            "  Navigate to a project initialized with 'specflow init'.",
            title="[yellow]Source Repository Detected[/yellow]",
            border_style="yellow",
            padding=(1, 2),
        )
    ...
    if is_existing_project(project_path) and not has_constitution(project_path):
        ...
        if sys.stdin.isatty():
            ...
            console.print(
                "[yellow]Run 'specflow init --here' in interactive mode to add a constitution[/yellow]"
            )
        console.print()
...
def _upgrade_backlog_md(dry_run: bool = False) -> tuple[bool, str]:
    ...
    if not current_version:
        return False, "backlog-md not installed (use 'specflow backlog install')"
...
def _list_jp_spec_kit_versions() -> None:
    ...
    console.print(
        "[dim]Use 'specflow upgrade-tools --version X.X.X' to install a specific version[/dim]"
    )
...
def upgrade_tools(
    ...
    It does NOT upgrade repository templates (use 'specflow upgrade-repo' for that).
    ...
    Examples:
        specflow upgrade-tools                    # Upgrade all tools
        specflow upgrade-tools -c jp-spec-kit    # Upgrade only jp-spec-kit
        specflow upgrade-tools -c spec-kit       # Upgrade only spec-kit templates
        specflow upgrade-tools -c backlog        # Upgrade only backlog-md
        specflow upgrade-tools --dry-run         # Preview what would be upgraded
        specflow upgrade-tools --version 0.2.325 # Install specific version
        specflow upgrade-tools --list-versions   # Show available versions

    See also:
        specflow upgrade-repo    # Upgrade repository templates
    """
    show_banner()
...
def _run_upgrade_repo(dry_run: bool = False) -> None:
    ...
    """
    console.print(
        "[yellow]Note:[/yellow] For full options, use 'specflow upgrade-repo' directly.\n"
    )
    console.print(
        "[dim]This simplified mode upgrades to latest versions. "
        "Use 'specflow upgrade-repo --help' for version pinning options.[/dim]\n"
    )
    ...
    console.print("  specflow upgrade-repo")
    console.print()
...
def upgrade(
    ...
    Examples:
        specflow upgrade              # Interactive: choose what to upgrade
        specflow upgrade --tools      # Same as 'specflow upgrade-tools'
        specflow upgrade --repo       # Same as 'specflow upgrade-repo'
        specflow upgrade --all        # Upgrade both tools and repo
    ...
    if backlog_version:
        ...
        if recommended_version:
            ...
            if backlog_version == recommended_version:
                ...
                console.print(
                    "[dim]Run 'specflow backlog upgrade' to sync to recommended version[/dim]"
                )
...
def dev_setup(
    ...
    projects, use 'specflow init' instead.

    Examples:
        specflow dev-setup           # Set up for development
        specflow dev-setup --force   # Recreate symlinks if they exist
    """
    show_banner()
        ...
        if not dry_run:
            ...
            next_steps.extend(
                [
                    ...
                    "[dim]Use 'specflow tasks generate' to regenerate tasks from spec[/dim]",
                ]
            )
...
def config_validation(
    ...
    if not workflow_path.exists():
        ...
        console.print("[dim]Run 'specflow init' first to create a project[/dim]")
        raise typer.Exit(1)
...
def security_scan(
    ...
    Examples:
        specflow security scan                           # Scan current directory
        specflow security scan /path/to/code             # Scan specific path
        specflow security scan --tool semgrep            # Specify scanner
        specflow security scan --fail-on critical        # Fail only on critical
        specflow security scan --format json -o out.json # JSON output to file
    """
    import json
...
def security_triage(
    ...
    Examples:
        specflow security triage findings.json
        specflow security triage findings.json --min-severity high
    """
...
def security_fix(
    ...
    Examples:
        specflow security fix SEMGREP-001 --dry-run
        specflow security fix SEMGREP-001 --apply
    """
...
def security_audit(
    ...
    Examples:
        ...
        specflow security audit --format html --output audit.html
        specflow security audit --compliance owasp
    """
...
def constitution_validate_cmd(
    ...
    if not constitution_path.exists():
        ...
        console.print("[yellow]Tip:[/yellow] Run 'specflow init --here' to create one")
        raise typer.Exit(1)
...
def constitution_version_cmd(
    ...
    if not constitution_path.exists():
        ...
        console.print("\n[yellow]Run 'specflow init --here' to create one[/yellow]")
        raise typer.Exit(1)
...
def constitution_diff(
    ...
    if not constitution_path.exists():
        ...
        console.print(f"  specflow init --here --constitution {tier}")
        raise typer.Exit(1)
    ...
    console.print(
        "\n[dim]Tip: Use 'specflow constitution merge' to update from template[/dim]"
    )
...
def workflow_validate(
    ...
    Examples:
        specflow workflow validate                    # Validate default config
        specflow workflow validate --file custom.yml  # Validate custom config
        specflow workflow validate --verbose          # Show detailed output
        specflow workflow validate --json             # JSON output for CI
    """
    import json
    from pathlib import Path
...
def main():
    """Primary entry point for jpspecify CLI."""
    app()


def main_deprecated():
    """Deprecated entry point for specify CLI (deprecated alias).

    This function is called when users invoke the old 'specify' command.
    It shows a deprecation warning and then calls the main function.
    """
    import sys

    # Only show warning if invoked directly (not during tests)
    if not any("pytest" in arg for arg in sys.argv):
        console = Console(stderr=True)
        console.print(
            "[yellow]⚠️  WARNING: The 'specify' command is deprecated.[/yellow]",
        )
        console.print(
            "[yellow]   Please use 'specflow' instead.[/yellow]",
        )
        console.print(
            "[dim]   This command will be removed in version 2.0.0[/dim]",
        )
        console.print(
            "[dim]   Migration guide: https://github.com/jpoley/jp-spec-kit/blob/main/docs/guides/migration-to-specflow.md[/dim]",
        )
        console.print()

    # Call the main function to execute the command
    main()
