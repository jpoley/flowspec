...
class ToolManager:
    ...
    def _install_binary(self, config: ToolConfig) -> InstallResult:
        ...
        try:
            ...
            # FIX #10: Catch specific security exceptions and re-raise after cleanup
    ...
    def _safe_extract_archive(self, archive_path: Path, dest_dir: Path) -> None:
        ...
        Security:
            ...
            - FIX #8: Detects and validates symlinks before resolving
        ...
        # FIX #4: Use .suffix.lower() for case-insensitive extension checking
        if archive_path.suffix.lower() == ".zip":
            with zipfile.ZipFile(archive_path, "r") as zf:
                ...
                for member in zf.namelist():
                    ...
                    # Use relative_to() for path validation (not string prefix)
            ...
            # FIX #4: Add pre-extraction validation for tar archives
            try:
                with tarfile.open(archive_path, "r:*") as tf:
                    ...
                    for member in tf.getmembers():
                        member_count += 1
                        # FIX #7: Check file count limit during pre-extraction
                        if member_count > self.MAX_ARCHIVE_FILES:
                            raise RuntimeError(
                        ...
                        # FIX #8: Detect symlinks and validate before resolving
                        if member.issym() or member.islnk():
                            # Symlinks could be used to escape destination
                            logger.warning(
                                f"Archive contains symlink (potential security risk): {member.name}"
                            )
            ...
            for item in dest_dir.rglob("*"):
                file_count += 1
                # FIX #6: File count limit check happens here for non-zip/tar archives
                ...
                try:
                    item_resolved.relative_to(dest_dir_resolved)
                except ValueError:
                    # FIX #2: Remove the suspicious file or directory
                    if item.is_file():
                        item.unlink()
                    elif item.is_dir():
                        shutil.rmtree(item)
