...
class ToolManager:
    ...
    def _extract_zip_safely(self, archive_path: Path, dest_dir: Path) -> None:
        ...
        with zipfile.ZipFile(archive_path, "r") as zf:
            ...
            # Issue #2: Post-extraction check - collect ALL symlinks, remove all, then raise
            symlinks = [p for p in dest_dir.rglob("*") if p.is_symlink()]
            if symlinks:
    ...
    def _validate_url_template(
        ...
        if not version:
            ...
            # Issue #8: Substitute version with error handling
            try:
                final_url = url_template.format(version=version)
            except KeyError as e:
                return (False, f"URL template missing placeholder: {e}")
