...
#   - "semgrep 1.0.0" (extra text, not anchored)
...
class ToolManager:
    ...
    def install(
        ...
        Returns:
            ...
            On failure, returns InstallResult with success=False and error_message.
        """
            ...
            def __enter__(self):
                ...
                while True:
                    try:
                        ...
                        fcntl.flock(
                            self.file_handle.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB
                        )
                        return self
                    except OSError:
    ...
    def _install_binary(self, config: ToolConfig) -> InstallResult:
        ...
        is_valid, error_msg = self._validate_url_template(
            url_template, config.version or ""
        )
        if not is_valid:
            return InstallResult(
                success=False,
        ...
        url = url_template.format(version=config.version or "")
        dest_dir = self.cache_dir / config.name / (config.version or "latest")

        # Acquire file-based lock for concurrent installation protection
        try:
            with self._acquire_install_lock(config.name, config.version or "latest"):
    ...
    def _install_binary_locked(
        ...
        try:
            ...
            # FIX #9: Remove redundant exists() check - is_file() is sufficient
            ...
            logger.error(f"Security error during binary install: {e}")
            ...
            logger.error(f"Filesystem error during binary install: {e}")
            # Clean up partial installation on any failure
            if dest_dir.exists():
                try:
                    shutil.rmtree(dest_dir)
    ...
    def _safe_extract_archive(self, archive_path: Path, dest_dir: Path) -> None:
        ...
        Raises:
            RuntimeError: If archive contains suspicious paths or too many files.
        """
        # Use .suffix.lower() for case-insensitive extension checking
        if archive_path.suffix.lower() == ".zip":
            with zipfile.ZipFile(archive_path, "r") as zf:
                ...
                for member in zf.namelist():
                    ...
                    # Check if path would escape using string prefix
                    try:
                ...
                # Post-extraction: check for and remove symlinks
                for extracted_path in dest_dir.rglob("*"):
                    if extracted_path.is_symlink():
                        logger.warning(
                            f"Removing symlink from zip archive (security risk): {extracted_path}"
                        )
                        extracted_path.unlink()
                        raise RuntimeError(
                            f"Zip archive contains symlink (not allowed): {extracted_path.name}"
                        )
        elif archive_path.suffix.lower() in [".tar", ".gz", ".tgz", ".bz2", ".xz"]:
            # Add pre-extraction validation for tar archives
            try:
                with tarfile.open(archive_path, "r:*") as tf:
                    ...
                    for member in tf.getmembers():
                        ...
                        if member.issym() or member.islnk():
                            # Symlinks could be used to escape destination
                            logger.warning(
                                f"Tar archive contains symlink (potential security risk): {member.name}"
                            )
                            # Validate symlink target doesn't escape - WITHOUT following it
                            if member.linkname:
                                # Check for absolute path in link target
                                if Path(member.linkname).is_absolute():
                                    raise RuntimeError(
                                        f"Archive contains symlink to absolute path: {member.linkname}"
                                    )
                                # Check for path traversal in link target
                                if ".." in Path(member.linkname).parts:
                                    raise RuntimeError(
                                        f"Archive contains symlink with path traversal: {member.linkname}"
                                    )
            ...
            # Note: shutil.unpack_archive handles the extraction.
            # The context manager (if used) will clean up on exceptions.
            ...
            for item in dest_dir.rglob("*"):
                file_count += 1
                # File count limit check for non-zip/tar archives
    ...
    def _validate_url_template(
        ...
        # Check for malformed URLs (double slashes except in protocol)
        ...
        if len(parts) == 2:
            ...
            if "//" in rest:
                return (
                    False,
                    f"Malformed URL after substitution: {final_url} (contains //)",
                )
            ...
            for item in self.cache_dir.iterdir():
                if item.is_dir():
                    try:
                        ...
                        # Ignore individual cache entries that cannot be accessed
                        logger.warning(f"Could not read cache entry {item}: {e}")
                        continue
        except (OSError, PermissionError) as e:
            # If entire cache directory is inaccessible, return empty result
    ...
    def _find_in_cache(self, tool_name: str) -> Path | None:
        ...
        try:
            ...
            # Cannot access cache directory, return None
            logger.debug(f"Cannot access cache directory {tool_dir}: {e}")
            return None
    ...
    def _find_executable_recursive(
        ...
        try:
            ...
            # Ignore directories/files that cannot be accessed, but log for debugging
    ...
    def _get_size_mb(self, path: Path) -> float:
        ...
        # Use os.walk() instead of rglob for efficiency
        total = 0
        try:
            for dirpath, _, filenames in os.walk(path):
                for filename in filenames:
                    ...
                    try:
                        ...
                        # Skip files that cannot be accessed (permissions, broken symlinks, etc.)
                        continue
        except (OSError, PermissionError) as e:
            # Cannot walk directory tree, return 0
            logger.debug(f"Cannot calculate size for {path}: {e}")
