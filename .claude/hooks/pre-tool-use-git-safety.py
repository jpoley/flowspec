...
DANGEROUS_GIT_PATTERNS = [
    ...
    (r"git\s+push.*\s+\+\w+", "Force push syntax (+branch) can overwrite remote history"),
    ...
    (r"git\s+clean\s+-fd", "git clean -fd will delete untracked files permanently"),
    (r"git\s+clean\s+-f\s+-d", "git clean will delete untracked files permanently"),
]
    ...
    for pattern, reason in DANGEROUS_GIT_PATTERNS:
        if re.search(pattern, command, re.IGNORECASE):
            ...
            if "rebase -i" in command_lower:
                additional_context += "Interactive commands are not supported in Claude Code. "
            ...
            if re.search(r"(push.*--force|push.*-f|push.*\+)", command, re.IGNORECASE):
                if re.search(r"(main|master)", command, re.IGNORECASE):
                    return True, "Force push to main/master branch detected", additional_context + "\n\nForce pushing to main/master is highly discouraged."
