...
**Generated**: YYYY-MM-DD by `/flow:constitution` command
...
- Re-run `/flow:constitution` after major tooling changes
```
...
4. Commit both files:
   ...
   Generated via /flow:constitution command"

💡 TIP: If tier seems wrong, override with:
   /flow:constitution --tier {light|medium|heavy}
```
...
- Suggest running `/flow:constitution` again after setup
