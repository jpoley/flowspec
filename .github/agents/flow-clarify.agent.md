...
1. Run `{SCRIPT}` from repo root **once** (combined `--json --paths-only` mode / `-Json -PathsOnly`). Parse minimal JSON payload fields:
   ...
   - If JSON parsing fails, abort and instruct user to re-run `/spec.flowspec` or verify feature branch environment.
...
- If spec file missing, instruct user to run `/spec.flowspec` first (do not create a new spec here).
