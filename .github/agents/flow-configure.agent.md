...
if [ ! -f "flowspec_workflow.yml" ] && [ ! -f "flowspec_workflow.yml" ] && [ ! -d ".claude" ]; then
  ...
  echo "Run '/flow:init' first to initialize the project"
  exit 1
fi
```
...
4. Suggest running `/flow:constitution` for full customization:
   ```
   Constitution template installed.

   For full customization with auto-detected tech stack, run:
     /flow:constitution --tier {selected_tier}
   ```
...
• Reconfigure anytime: /flow:configure
• Change just role: /flow:configure --role <new_role>
• Reset validation modes: /flow:configure --all
```
...
- **Not in project directory**: Show error and suggest `/flow:init`
...
flowspec telemetry track-role "{selected_role}" --command /flow:configure -q
```
