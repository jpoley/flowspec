# Platform Engineering Design: /flow:security Commands
...
The `/flow:security` feature is designed to maintain DORA Elite classification while introducing security quality gates:
...
CACHE_PATH=".flowspec/cache/security/${CACHE_KEY}.json"
```
...
The `/flow:security` commands integrate at three points in the development workflow:
...
graph LR
    ...
    H --> L[Developer Runs /flow:security fix]
    L --> M[Apply Patches]
    M --> D
```
...
**Configuration:**
```yaml
# flowspec_security.yml (project-level config)
security_gates:
  pre_commit:
    enabled: true
...
def ensure_semgrep_installed():
    ...
    cache_dir = Path.home() / ".cache" / "flowspec" / "tools"
...
CACHE_SIZE=$(du -sh ~/.cache/flowspec/tools | awk '{print $1}')
...
```mermaid
graph TD
    A[Code Commit] --> B[/flow:security scan]
    B --> C{Parse Results}
    ...
    E --> I[/flow:security triage]
    I --> J{AI: True Positive?}
    J -->|Yes| K[/flow:security fix]
...
|--------|------|--------|---------|
| `flowspec_security_scan_duration_seconds` | Histogram | tool, scan_type, outcome | Track scan performance |
| `flowspec_security_findings_total` | Counter | severity, cwe_id, tool | Vulnerability trends |
| `flowspec_security_triage_duration_seconds` | Histogram | model, finding_count | AI triage performance |
| `flowspec_security_triage_accuracy` | Gauge | model | AI triage correctness |
| `flowspec_security_fix_application_rate` | Gauge | severity | Fix adoption rate |
| `flowspec_security_false_positive_rate` | Gauge | tool | Tool accuracy |
| `flowspec_security_gate_blocks_total` | Counter | severity, gate_type | Pipeline impact |
...
avg_over_time(
  flowspec_security_findings_total{severity="critical"}[7d]
)
```
...
histogram_quantile(0.95,
  sum(rate(flowspec_security_scan_duration_seconds_bucket[5m])) by (le, tool)
)
```
...
avg_over_time(
  flowspec_security_triage_accuracy[1h]
)
```
...
(
  flowspec_security_fix_application_rate{severity="critical"}
  /
  flowspec_security_findings_total{severity="critical"}
) * 100
```
...
groups:
  - name: flowspec_security
    interval: 5m
    rules:
      - alert: CriticalVulnerabilityFound
        expr: flowspec_security_findings_total{severity="critical"} > 0
        for: 5m
        annotations:
      ...
      - alert: SecurityScanPerformanceDegraded
        expr: |
          histogram_quantile(0.95,
            sum(rate(flowspec_security_scan_duration_seconds_bucket[10m])) by (le, tool)
          ) > 300
        for: 15m
        annotations:
      ...
      - alert: AITriageAccuracyLow
        expr: flowspec_security_triage_accuracy < 0.8
        for: 1h
        annotations:
...
# Install Flowspec and security tools
...
```bash
# Build
docker build -t flowspeckit/security-scanner:latest -f docker/security-scanner.Dockerfile .

# Publish to GHCR
docker tag flowspeckit/security-scanner:latest ghcr.io/yourusername/flowspeckit-security-scanner:1.0.0
docker push ghcr.io/yourusername/flowspeckit-security-scanner:1.0.0
```
...
docker run --rm -v $(pwd):/src \
  ghcr.io/yourusername/flowspeckit-security-scanner:1.0.0 \
...
backlog task create "Setup CI/CD Security Scanning Pipeline" \
  -d "Configure GitHub Actions for security scanning integration with /flow:security commands" \
...
backlog task create "Implement Security Scanning Observability" \
  -d "Add Prometheus metrics, structured logging, and dashboards for /flow:security" \
...
backlog task create "Create Pre-commit Hook Configuration for Security Scanning" \
  ...
  --ac "Create .pre-commit-config.yaml template with flowspec-security hook" \
...
backlog task create "Implement Security Policy as Code Configuration" \
  -d "Build .flowspec/security-policy.yml parser and enforcement engine" \
...
**Test Projects:**
1. **Small**: 10K LOC Python project (e.g., Flowspec itself)
...
- Target: 5 beta users (Flowspec contributors)
- Scope: `/flow:security scan` only (Semgrep)
- Goal: Validate tool orchestration and performance

**Phase 2: Beta (Week 3-4)**
- Target: 20 beta users (public repo projects)
- Scope: Add `/flow:security triage` (AI-powered)
- Goal: Validate AI triage accuracy and usability

**Phase 3: GA (Week 5-6)**
- Target: All Flowspec users
