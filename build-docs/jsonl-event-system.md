...
| `action` | object | Action invocation details | See Action Object |
| `security` | object | Security operation details | See Security Object |
...
| `lifecycle.paused` | Agent execution paused | `break` action, awaiting approval |
| `lifecycle.resumed` | Agent execution resumed | `resume` action after pause |
...
| `git.conflict_resolved` | Merge conflicts resolved | After `resolve_conflicts` action |
...
### 10. Action Namespace (`action.*`)

Observations of action lifecycle. See [Action & Event System](action-system.md) for the complete action vocabulary. **New in v1.1.0.**

**Key points**:
- Actions are requests (can be denied by humans, agents, or programs)
- Events are observations (facts that something occurred)
- Denied actions emit no events; accepted actions always emit a terminal event

| Event Type | Description | When Emitted |
|------------|-------------|--------------|
| `action.invoked` | Action request was accepted, execution began | After request accepted |
| `action.succeeded` | Action execution completed successfully | Successful completion |
| `action.failed` | Action execution failed with error | Runtime failure |
| `action.aborted` | Action execution was interrupted | User/system abort |

**Guaranteed terminal event**: For every accepted action, exactly one of `succeeded`, `failed`, or `aborted` is always emitted.

**Example - Action Invocation:**
```json
{"version":"1.1.0","event_type":"action.invoked","timestamp":"2025-12-13T21:00:00.000Z","agent_id":"@backend-engineer","message":"Invoking validate action","context":{"task_id":"task-123"},"action":{"verb":"validate","domain":"agent","category":"decide","input":{"subject":{"ref":"src/api.py"},"ruleset":{"ref":"eslint"}}},"correlation":{"trace_id":"trace-xyz","span_id":"span-010"}}
```

**Example - Action Success with Side-Effect:**
```json
{"version":"1.1.0","event_type":"action.succeeded","timestamp":"2025-12-13T21:00:05.000Z","agent_id":"@backend-engineer","message":"validate completed: 0 issues found","context":{"task_id":"task-123"},"action":{"verb":"validate","duration_ms":5000,"output":{"ok":true,"issues":[]}},"correlation":{"trace_id":"trace-xyz","span_id":"span-010"}}
```

**Example - Action Failure:**
```json
{"version":"1.1.0","event_type":"action.failed","timestamp":"2025-12-13T21:00:10.000Z","agent_id":"@backend-engineer","message":"lint action failed: 5 errors found","context":{"task_id":"task-123"},"action":{"verb":"lint","duration_ms":3000,"error":{"code":"LINT_FAILURE","message":"5 errors found","issues":[]}}}
```

### 11. Security Namespace (`security.*`)

Security-specific events for signing, attestation, and compliance. **New in v1.1.0.**

| Event Type | Description | When Emitted |
|------------|-------------|--------------|
| `security.artifact_signed` | Artifact was signed | After `sign` action |
| `security.attestation_created` | SLSA attestation created | After attestation |
| `security.vulnerability_found` | Vulnerability discovered | SAST/SCA scan |

**Example - Artifact Signed:**
```json
{"version":"1.1.0","event_type":"security.artifact_signed","timestamp":"2025-12-13T21:30:00.000Z","agent_id":"system","message":"Container image signed","context":{"task_id":"task-123"},"security":{"artifact":{"type":"container","ref":"ghcr.io/org/app:v1.0.0"},"signature":{"algorithm":"cosign","key_id":"KEY-001","signature_ref":"sig-abc123"},"attestation":{"type":"in-toto","predicate":"slsa.v1"}}}
```
...
### Summary Table (60 Event Types)

| Namespace | Event Type | Status Equivalent | Description |
|-----------|------------|-------------------|-------------|
| lifecycle | `lifecycle.started` | started | Agent begins execution |
| lifecycle | `lifecycle.paused` | waiting | Agent execution paused |
| lifecycle | `lifecycle.resumed` | progress | Agent execution resumed |
...
| git | `git.conflict_resolved` | progress | Merge conflicts resolved |
...
| action | `action.invoked` | progress | Action execution started |
| action | `action.succeeded` | progress | Action completed successfully |
| action | `action.failed` | error | Action failed with error |
| action | `action.aborted` | - | Action was aborted |
| security | `security.artifact_signed` | - | Artifact was signed |
| security | `security.attestation_created` | - | SLSA attestation created |
| security | `security.vulnerability_found` | - | Vulnerability discovered |
...
```json
{
  ...
  "properties": {
    ...
    "action": {
      "type": "object",
      "properties": {
        "verb": {"type": "string", "description": "Action verb from action-system.md"},
        "domain": {"type": "string", "enum": ["human", "agent", "code", "system"]},
        "category": {"type": "string"},
        "input": {"type": "object", "description": "Input parameters"},
        "output": {"type": "object", "description": "Output values"},
      ...
      "description": "Action invocation details (integrates with action-system)"
    },
    "security": {
      "type": "object",
      "properties": {
...
- [Action & Event System](action-system.md) - Unified vocabulary for actions and events
