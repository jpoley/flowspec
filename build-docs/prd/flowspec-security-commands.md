# PRD: /flow:security Commands - Comprehensive Security Scanning for SDD Workflow

**Feature**: flowspec-security-commands
**Date**: 2025-12-02
...
Introduce `/flow:security` commands that seamlessly integrate security scanning into the SDD workflow through intelligent tool orchestration, AI-assisted triage, and automated fix suggestions.

**Core Commands:**
```bash
/flow:security scan      # Orchestrate Semgrep (+ optional CodeQL) scans
/flow:security audit     # Generate comprehensive security audit reports
/flow:security triage    # AI-assisted vulnerability prioritization and triage
/flow:security fix       # Generate fix suggestions with code patches
```

**Strategic Decision**: Build native implementation (not fork Raptor) using Raptor's proven patterns as reference. This maintains architectural coherence with JP Flowspec while avoiding the 6GB DevContainer and privileged execution requirements of Raptor.
...
| **Leading Indicators** | | | |
| `/flow:security scan` adoption rate | >80% of projects | 6 weeks | Projects using command / Total projects |
...
**Strategic Alignment:**
- **SLSA L3 Compliance**: Native security scanning supports JP Flowspec's mission to enable production-grade quality from day one
...
- **Tech Stack**: Python/TypeScript, uses JP Flowspec for side projects
...
```mermaid
graph LR
    A[Completes feature implementation] --> B[Runs /flow:security scan]
    B --> C[Semgrep finds 5 issues]
    C --> D[Runs /flow:security triage]
    D --> E[AI ranks: 2 critical, 3 false positives]
    E --> F[Runs /flow:security fix for critical issues]
...
```mermaid
graph LR
    A[Configures /flow:security in CI] --> B[Teams use /flow:implement]
    B --> C[Auto-triggers /flow:security scan]
    C --> D[Blocks PR if critical issues found]
    D --> E[Developers run /flow:security fix]
...
**Key Moments:**
- **Configuration**: One-time setup in `flowspec_workflow.yml` enables enforcement for all teams
...
**Acceptance Criteria:**
- [ ] AC1.1: Running `/flow:security scan` executes Semgrep with OWASP ruleset by default
...
**Acceptance Criteria:**
- [ ] AC2.1: `/flow:security triage` reads scan results from previous scan
...
**Acceptance Criteria:**
- [ ] AC3.1: `/flow:security fix` generates patches for triaged True Positive findings
...
**Acceptance Criteria:**
- [ ] AC4.1: `/flow:security audit` generates report using template `templates/security-report-template.md`
...
**Acceptance Criteria:**
- [ ] AC5.1: `/flow:security scan` can run in CI environment without interactive prompts
...
- **Scenario**: User runs `/flow:security scan --tool codeql` on private repository without CodeQL license
...
- **Scenario**: User runs `/flow:security triage` without running `scan` first
- **Expected Behavior**: Command displays helpful error and suggests running scan
- **Error Message**: "❌ No scan results found. Run `/flow:security scan` first to generate findings."
...
**Question**: Will developers and security engineers actually use `/flow:security` commands?

**Risk Level**: Medium
**Confidence**: 60% (needs validation)

**Evidence For:**
- ✅ Existing `/flow:validate` is actively used (50+ executions in beta)
...
- ✅ User interviews show 80% of JP Flowspec users want integrated security
...
**Evidence For:**
- ✅ Command structure follows existing `/flow:*` conventions (low learning curve)
...
3. **Guided Workflows**: Command suggests next steps (e.g., "Run `/flow:security triage` to prioritize findings")
...
- ✅ JP Flowspec already has MCP integration for AI reasoning
...
- ✅ Aligns with JP Flowspec mission (production-grade quality)
...
### FR1: Security Scanner Orchestration (`/flow:security scan`)
...
# Specify scanner
/flow:security scan --tool semgrep
/flow:security scan --tool codeql
/flow:security scan --tool all

# Custom ruleset
/flow:security scan --config custom-rules.yml

# Incremental scan (only changed files)
/flow:security scan --incremental

# Fail on severity threshold
/flow:security scan --fail-on critical,high

# Output format
/flow:security scan --format json|markdown|sarif

# Verbose output
/flow:security scan --verbose
```
...
### FR2: AI-Powered Vulnerability Triage (`/flow:security triage`)
...
/flow:security triage --interactive

# Specify scan results file
/flow:security triage --scan-results path/to/results.json

# Filter by severity
/flow:security triage --min-severity high

# Output format
/flow:security triage --format json|markdown
```
...
### FR3: Automated Fix Generation (`/flow:security fix`)
...
### FR4: Security Audit Report Generation (`/flow:security audit`)
...
/flow:security audit --format markdown|html|pdf

# Include only specific severities
/flow:security audit --severity critical,high

# Compliance mode (extra detail for auditors)
/flow:security audit --compliance soc2|iso27001|hipaa
```
...
**Description**: Integrate `/flow:security` commands into existing SDD workflow.

**Sub-Requirements:**

#### FR5.1: Workflow State Transitions
Potential integration points:
1. **Option A: Extend `/flow:validate`** - Security scan becomes sub-phase of validation
...
Example:
```bash
/flow:security triage --create-tasks
```
...
- [ ] Verify with `/flow:security scan` (finding FINDING-001 resolved)
...
Create `.flowspec/security-config.yml`:
...
- Users can provide custom Semgrep rules in `.flowspec/security-rules/`
...
- Example:
  ...
  Or allow automatic installation:
    /flow:security scan --auto-install
  ```
...
- **Strategy**: Download-on-demand with caching
  - First run: Download Semgrep (~50MB), cache in `~/.flowspec/tools/`
...
- **Command Reference**: `docs/reference/flowspec-security-commands.md`
  - All command flags
...
| **task-215** | Implement /flow:security CLI Commands | High | security, implement, cli |
| **task-216** | Integrate /flow:security with Workflow and Backlog | High | security, implement, workflow |
...
5. **task-216: Workflow Integration** - Wire into flowspec ecosystem
...
6. **task-217: Configuration System** - User customization
   - `.flowspec/security-config.yml` support
...
2. Run `/flow:security scan` on each
...
Before the `/flow:security` commands can be considered "Done," the following criteria must be met:

#### AC1: Core Functionality
- [ ] `/flow:security scan` successfully scans Python, JavaScript, Go, and TypeScript projects
- [ ] Scan results include severity, CWE ID, OWASP category, file location, and code snippet
- [ ] `/flow:security triage` classifies findings with >85% accuracy (validated against benchmark)
- [ ] `/flow:security fix` generates syntactically valid patches for 80%+ of findings
- [ ] `/flow:security audit` generates complete report using security-report-template.md
...
#### AC4: Integration
- [ ] Commands integrate with flowspec_workflow.yml (documented integration options)
...
- [ ] Command Reference published at `docs/reference/flowspec-security-commands.md`
...
1. **Semgrep Integration**:
   ...
   - Test custom ruleset loading from `.flowspec/security-rules/`
...
1. **Happy Path: Solo Developer Workflow**:
   - User runs `/flow:security scan` on new Python project
   - Scan finds 5 vulnerabilities (3 TP, 2 FP)
   - User runs `/flow:security triage`
   - AI correctly classifies 3 as TP, 2 as FP
   - User runs `/flow:security fix` for 3 TP findings
   - User applies patches and re-scans
   - Scan now shows 0 vulnerabilities
   - User runs `/flow:security audit` to generate report

2. **CI/CD Integration Workflow**:
   - GitHub Actions workflow triggers on PR
   - Workflow runs `/flow:security scan --fail-on critical,high`
...
3. **Error Recovery Workflow**:
   - User runs `/flow:security triage` without prior scan
...
3. **Bundled**: Include in JP Flowspec distribution (increases size)

**Version Requirement**: Semgrep >=1.45.0 (for SARIF output support)

**Constraints**:
- Requires Python 3.7+ (JP Flowspec already requires 3.11+, so compatible)
...
- **Network dependency**: Cache Semgrep Registry rules in `~/.flowspec/cache/semgrep-rules/`
...
- ⚠️ **Verdict**: Defer to post-MVP until licensing is clarified for JP Flowspec users
...
**Options**:
1. **Anthropic Claude API** (preferred for JP Flowspec users)
...
### Internal Dependencies (JP Flowspec)
...
**Dependency**: `flowspec_workflow.yml` schema must support security workflow states

**Requirement**: Either extend existing `/flow:validate` or add new state "Security Audited"

**Decision Point**: Resolved in task-220 (determine if `/flow:security` is standalone or part of validate)
...
**Background**: task-198 ("Design Unified Vulnerability Scanner - Trivy + Snyk Integration") is currently In Progress and overlaps with `/flow:security` scope.
...
- `/flow:security scan` becomes the unified entry point
...
- `/flow:security scan` → Code vulnerabilities (SAST)
...
- Create `/flow:security scan --all` to invoke both Semgrep and Trivy/Snyk
...
**JP Flowspec License**: MIT

**Analysis**:
- ✅ **Compatible**: LGPL allows linking as external tool (subprocess invocation)
- ✅ **No code incorporation**: JP Flowspec does not include Semgrep source code
...
- JP Flowspec itself is OSS and not subject to restrictions, but Semgrep may be
...
1. Track vulnerabilities found by `/flow:security scan` in development
...
**Data Source**: Telemetry from `/flow:security` commands (with user consent)

### Leading Indicators (Predict North Star Success)

#### LI1: `/flow:security scan` Adoption Rate

**Definition**: Percentage of JP Flowspec projects that have run `/flow:security scan` at least once
...
**Data Source**: User feedback via `/flow:security triage --interactive` or manual FP reports
...
**Rationale**: Quantifies ROI for enterprises evaluating JP Flowspec.
...
**Definition**: "How likely are you to recommend `/flow:security` commands to a colleague?" (0-10 scale)
...
**Data Source**: In-app survey after `/flow:security triage` command
...
- AI triage feedback: After first `/flow:security triage` run
- Fix quality: After first `/flow:security fix` run
...
| **task-215** | Implement /flow:security CLI Commands | User interface |
| **task-216** | Integrate /flow:security with Workflow and Backlog | Workflow integration |
...
The `/flow:security` feature uses a three-layer architecture to support both interactive AI-assisted workflows and automated CI/CD pipelines.

### Architecture Diagram

```
┌─────────────────────────────────────────────────────────────┐
│  /flow:security (Slash Commands)                         │
│  ─────────────────────────────────────────────────────────  │
...
This section documents the relationship between JP Flowspec's `/flow:security` and the [Raptor framework](https://github.com/gadievron/raptor).
...
| Capability | Raptor | flow:security | Status |
|------------|--------|-----------------|--------|
| **Static Analysis** | | | |
...
**Status**: ✅ Complete - Ready for Planning Phase (`/flow:plan`)
...
5. Proceed to `/flow:plan` for architecture and platform design
