...
flowchart TD
    ...
    %% Commands
    CMD_SPECIFY["/flow:specify<br/>Create/Update Feature Specs"]
    CMD_PLAN["/flow:plan<br/>Execute Planning Workflow"]
    CMD_RESEARCH["/flow:research<br/>Research & Business Validation"]
    CMD_IMPLEMENT["/flow:implement<br/>Feature Implementation"]
    CMD_VALIDATE["/flow:validate<br/>QA & Validation"]
    CMD_OPERATE["/flow:operate<br/>Operations & SRE"]
...
| Element | Description |
|---------|-------------|
| 🔵 **Blue Rectangles** | `/flowspec` Slash Commands (entry points) |
...
- **/flow:plan**: Software Architect + Platform Engineer (2 agents in parallel)
- **/flow:implement Phase 1**: Frontend + Backend + AI/ML Engineers (up to 3 agents in parallel)
- **/flow:validate Phase 1**: QA + Security (2 agents in parallel)

#### Sequential Execution
Agents execute one after another, with each depending on the previous:
- **/flow:research**: Researcher → Business Validator (2 agents sequential)
- **/flow:implement Phase 2**: Code reviews after implementation (sequential)
- **/flow:validate Phases**: Testing → Documentation → Release Management (sequential)

#### Single Agent
Commands with one specialized agent:
- **/flow:specify**: Product Requirements Manager (1 agent)
- **/flow:operate**: Site Reliability Engineer (1 agent)
...
|---------|--------|-------------------|
| /flow:specify | 1 | Single |
| /flow:plan | 2 | Parallel |
| /flow:research | 2 | Sequential |
| /flow:implement | 3-5 | Multi-phase (parallel + sequential) |
| /flow:validate | 4 | Multi-phase (parallel + sequential + human gate) |
| /flow:operate | 1 | Single |
...
1. **Specification**: `/flow:specify` → Product Requirements Manager creates PRD
2. **Planning**: `/flow:plan` → Architect + Platform Engineer design system (parallel)
3. **Research** (optional): `/flow:research` → Validate market fit and feasibility
4. **Implementation**: `/flow:implement` → Engineers build + reviewers validate
5. **Validation**: `/flow:validate` → QA + Security + Docs + Release approval
6. **Operations**: `/flow:operate` → SRE establishes operational infrastructure

### Constitutional Development

The `/flow:plan` command builds out `/speckit.constitution` with:
