...
Building production-ready software requires expertise across multiple domains: product management, architecture, frontend, backend, security, testing, operations, and more. flowspec brings together specialized AI agents that work together seamlessly, enabling you to:
...
flowspec automates the software delivery lifecycle through six specialized workflows:
...
/flow:research Build a task management REST API

# 2. Create product specifications
/flow:specify Task management REST API with authentication

# 3. Design architecture and platform
/flow:plan Task management REST API

# 4. Implement the backend
/flow:implement Task management REST API backend
...
**Command:**
```bash
/flow:research AI-powered customer service chatbot with sentiment analysis and multi-language support
```
...
**Command:**
```bash
/flow:specify AI customer service chatbot based on research findings
```
...
**Command:**
```bash
/flow:plan AI customer service chatbot
```
...
**Command:**
```bash
/flow:implement AI customer service chatbot
```
...
**Command:**
```bash
/flow:validate AI customer service chatbot
```
...
**Input Format:**
```bash
/flow:research <project description>
```

**Example:**
```bash
/flow:research AI-powered code review tool for enterprise development teams
```
...
**Input Format:**
```bash
/flow:specify <feature description> [context from research]
```

**Example:**
```bash
/flow:specify Authentication system with OAuth2, SSO, and MFA based on enterprise security requirements
```
...
**Input Format:**
```bash
/flow:plan <project name> [PRD and requirements]
```

**Example:**
```bash
/flow:plan E-commerce checkout system with payment processing and inventory management
```
...
**Input Format:**
```bash
/flow:implement <feature description> [architecture and specs]
```

**Example:**
```bash
/flow:implement Real-time notification system with WebSocket support and push notifications
```
...
**Input Format:**
```bash
/flow:validate <feature name> [code and artifacts]
```

**Example:**
```bash
/flow:validate Payment processing API with PCI DSS compliance requirements
```
...
flowspec uses a sophisticated multi-agent orchestration system that coordinates specialized AI agents across the software delivery lifecycle.
...
```bash
/flow:research <idea>      # Start here
/flow:specify <idea>       # Then specify
...
```bash
/flow:specify Real-time collaborative document editor with operational transformation,
...
```bash
/flow:research Blockchain-based supply chain tracking for pharmaceutical industry
...
**Do:** Carefully review the PRD from `/flow:specify`
...
**Do:** Preserve ADRs from `/flow:plan` in your repository
```bash
...
/flow:implement Background job processing system

# For prototypes, skip operations
/flow:research → specify → plan → implement
...
**Note:** flowspec automatically runs agents in parallel when possible
- `/flow:plan`: Architecture + Platform (parallel)
- `/flow:implement`: Frontend + Backend + ML (parallel)
- `/flow:validate`: QA + Security (parallel)
...
```bash
# Instead of:
/flow:specify payment system

# Provide more context:
/flow:specify payment processing system supporting credit cards, ACH, and crypto,
...
**Root Cause:**
- Implementation didn't follow architecture from `/flow:plan`
- Requirements from `/flow:specify` not met

**Solution:**
1. Review architecture and requirements
2. Re-run `/flow:implement` with explicit architecture constraints
...
3. **Re-run validation**: After fixes, run `/flow:validate` again
...
4. Re-run `/flow:validate` with updated code
...
1. Provide tech-writer agent with more context:
   ```bash
   /flow:validate <feature> [include detailed API specs, architecture diagrams, test results]
   ```
...
1. **Specify technology choices in architecture phase:**
   ```bash
   /flow:plan E-commerce platform using Go backend, React frontend, PostgreSQL database
   ```
...
3. **Explicitly constrain in implementation:**
   ```bash
   /flow:implement API service in Go using Chi router and PostgreSQL with sqlx
   ```
...
flowspec transforms software delivery by orchestrating specialized AI agents across the complete lifecycle:
