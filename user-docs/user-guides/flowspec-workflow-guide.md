...
- [Diagram: /flowspec Workflow](#diagram-flowspec-workflow)
...
See [/flow:assess](#flowspecassess) for detailed complexity assessment.
...
1. **`/flow:specify`** - PM Planner creates implementation tasks in PRD section 6:
   ```bash
...
2. **`/flow:research`** - Researchers create follow-up tasks:
   ```bash
...
3. **`/flow:plan`** - Architects and Platform Engineers create design/infrastructure tasks:
   ```bash
   # Architecture tasks
...
![flowspec Workflow Diagram](../diagrams/flowspec-workflow.png)
...
**Version**: 1.0
**Maintained By**: Flowspec Project
