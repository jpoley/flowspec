...
- Run `/flow:init` after major tool changes
...
### Problem: `/flow:init` command not available in Claude

**Symptoms**:
- Typing `/flow:init` does nothing
...
If `/flow:init` unavailable, manually ask LLM:
...
.claude/commands/spec/constitution.md # /flow:init command
```
